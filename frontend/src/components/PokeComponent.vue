<template>
  <div class="font-sans bg-cover w-100% h-100%">
    <div class="">
      <h1
        class="font-black text-center text-2xl sm:text-3xl py-5 px-1 w-full bg-blue-500 text-white"
      >
        ポケモン剣盾 ステータス検索・比較
      </h1>
    </div>
    <div class="flex flex-wrap md:flex-nowrap">
      <div class="flex1 py-5 lg:pl-10" @click="oModal = false">
        <div
          class="px-4 py-2 m-5 shadow appearance-none border rounded-lg bg-white"
        >
          <div class="text-lg font-bold">自分のポケモン</div>
          <table class="">
            ...
            <table class="shadow appearance-none border">
              <tr class="border-b min-w-4/5">
                <th class="border-b w-16">種族値</th>
                <td></td>
              </tr>
              <tr class="border">
      ...
      <div class="flex1 py-5 lg:pr-10" @click="mModal = false">
        <div class="px-4 py-2 m-5 shadow appearance-none border rounded-lg">
          <div class="text-lg font-bold">相手のポケモン</div>
          <table class="">
            <div class="py-3">
              <td style="min-width: 250px">
                <v-select
...
export default {
  ...
  methods: {
    ...
    ImgSrc() {
      if (this.mPokemon !== "") {
        const mUrl = this.url + `${this.mPokemon.no}`;
        fetch(mUrl)
          .then((response) => {
            return response.json();
          })
          .then((pokemon) => {
            this.mImage = pokemon.sprites.front_default;
          });
      }
      if (this.oPokemon !== "") {
        const oUrl = this.url + `${this.oPokemon.no}`;
        fetch(oUrl)
          .then((response) => {
            return response.json();
          })
          .then((pokemon) => {
            this.oImage = pokemon.sprites.front_default;
          });
      }
    },
